...
{
    float k = 1.5; //Constante spring
    float d = 1.1; //Constante damper
...
void ArchivoObj::mostrar(GLuint& vbo,  GLuint& vbn)
{

    if(!estatico)springsAndDampers();
    vertices.clear();
    ...
    for (std::size_t i = 0; i < todosVertices.size(); i++)
        if(!estatico)todosVertices[i].actualizar();
...
void ArchivoObj::transformar(Transformacion t)
{
    for(std::size_t i=0 ; i<this->todosVertices.size() ; i++)
        todosVertices[i].posicion = t.nuevaPosicion(todosVertices[i].posicion);
}
